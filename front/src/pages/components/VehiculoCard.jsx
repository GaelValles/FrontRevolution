...
const VehiculoCard = ({ vehiculoId, onDelete }) => {
  ...
  useEffect(() => {
    const fetchVehiculoData = async () => {
      try {
        const data = await getCarroById(vehiculoId);
        setVehiculo(data);
      } catch (error) {
    ...
    fetchVehiculoData();
  }, [vehiculoId, getCarroById]);

  if (loading) {
    return (
      <div className="bg-black/80 backdrop-blur-xl border border-white/20 rounded-3xl p-6 animate-pulse">
